...
export function applyPointerMove(input, event, bounds = {}, worldBounds = bounds) {
  ...
  const worldWidth = worldBounds.width ?? width;
  const worldHeight = worldBounds.height ?? height;

  input.targetX = width > 0 ? clamp((localX / width) * worldWidth, 0, worldWidth) : 0;
  input.targetY = height > 0 ? clamp((localY / height) * worldHeight, 0, worldHeight) : 0;
