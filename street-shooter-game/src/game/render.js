...
function roundPercent(value, total, fallback = 0) {
  const safeTotal = clampNumber(total, 1);
  const safeValue = clampNumber(value, fallback);
...
function renderDecoration(anchor, index) {
  ...
  return `
    <div
      ...
      style="left:${roundPercent(anchor?.x, bounds.width)}; top:${roundPercent(anchor?.y, bounds.height)};"
      aria-hidden="true"
    ></div>
  `;
}
...
function renderWeaponPickup(pickup, index, language) {
  ...
  return `
    <div
      ...
      style="left:${roundPercent(pickup?.x, bounds.width)}; top:${roundPercent(pickup?.y, bounds.height)};"
    >
...
function renderPlayer(state, language) {
  ...
  return `
    <div
      ...
      style="left:${roundPercent(position.x, bounds.width)}; top:${roundPercent(position.y, bounds.height)};"
...
function renderEnemy(enemy, index) {
  ...
  return `
    <div
      ...
      style="left:${roundPercent(enemy?.x, bounds.width)}; top:${roundPercent(enemy?.y, bounds.height)};"
    >
...
function renderBullet(bullet, index) {
  ...
  return `
    <div
      ...
      style="left:${roundPercent(bullet?.x, bounds.width)}; top:${roundPercent(bullet?.y, bounds.height)};"
      aria-hidden="true"
    ></div>
  `;
}
  ...
  return `
    <main class="game-shell" data-game-state="${state?.gameOver ? "game-over" : "running"}">
      ...
      >
        ...
        <div class="street-world" style="width:100%; height:100%;">
