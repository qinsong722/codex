...
describe("createInitialState", () => {
  ...
  test("current weapon starts with the default pistol", () => {
    const state = createInitialState();

    expect(state.currentWeapon).toBe("pistol");
  });
  ...
  test("new rounds can start with different ammo pickup positions", () => {
    const firstRound = createInitialState(LANGUAGES.ZH, () => 0);
    const secondRound = createInitialState(LANGUAGES.ZH, () => 0.9);

    expect(firstRound.pickups.ammo).not.toEqual(secondRound.pickups.ammo);
  });
