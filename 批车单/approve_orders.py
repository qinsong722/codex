...
EDGE_PROFILE_ROOT = APP_DIR / "edge-profile"
...
class ApproveBot:
    def __init__(self) -> None:
        ...
        self.profile_dir = self.create_fresh_profile_dir()
    ...
    def create_fresh_profile_dir(self) -> Path:
        if EDGE_PROFILE_ROOT.exists():
            shutil.rmtree(EDGE_PROFILE_ROOT, ignore_errors=True)
        EDGE_PROFILE_ROOT.mkdir(parents=True, exist_ok=True)
        return Path(tempfile.mkdtemp(prefix="run-", dir=EDGE_PROFILE_ROOT))
    ...
    def dismiss_noise(self) -> None:
        ...
        clicked = False
        for xpath in xpaths:
            element = self.find_visible((By.XPATH, xpath), timeout=1)
            if not element:
                continue
            try:
                self.safe_click(element)
    ...
    def focus_code_input(self) -> None:
        ...
        try:
            ...
            self.system_click_code_input(code_input)
    ...
    def activate_browser_window(self) -> None:
        ...
        try:
            ...
            def enum_windows_proc(hwnd, lparam):
                ...
                if title in window_title and "Microsoft Edge" in window_title:
                    matched_hwnd = hwnd
                    return False
                return True
    ...
    def sleep_with_heartbeat(self, seconds: int) -> None:
        ...
        while remaining > 0:
            ...
            remaining -= 1
            elapsed += 1
            if elapsed % KEEPALIVE_INTERVAL_SECONDS == 0:
    ...
    def audit_page_looks_stuck(self) -> bool:
        try:
            return self.driver.execute_script(
                ...
                return Boolean(spinner) && hasApprovalTabs;
                """,
            )
        except WebDriverException:
            return False
...
def ensure_edge_exists() -> None:
    if not Path(EDGE_BINARY).exists():
        raise FileNotFoundError(f"未找到 Edge 浏览器: {EDGE_BINARY}")
